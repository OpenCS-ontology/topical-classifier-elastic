...
from config import TITLE_URI, ABSTRACT_URI, HAS_DISCIPLINE_URI, OCS_URI, PAPER_TYPE_URI, ORIGINAL_FILES_RESULT_DIR
from source.data.input_data_reading import input_file_to_graph, extract_object_literal_from_graph
...
def classify_input_files(
    ...
    for i in range(n_files):
        ...
        title, abstract = _parse_input_file(input_file_path, TITLE_URI, ABSTRACT_URI, file_format)

        _replace_title_abstract_placeholders(_query, title, abstract, title_placeholder, abstract_placeholder)

        query_result = find_n_best(es, index_name, _query, n)
        print(f"Query result for a file {file_name}:")
        print(query_result)
        
        if _decide_to_override(ask_to_override): continue
        print("Saving the result and updating the discipline with the best result.")
        
        best_concept, _ = _get_best_concept_and_score(query_result, labels_to_concept_names, label_colname)
        _override_and_move_input_file(input_file_path, best_concept, move_original, file_format)
...
def _parse_input_file(input_file_path: str, title_uri: str, abstract_uri: str, file_format: str):
        graph = input_file_to_graph(input_file_path, file_format)
        title = extract_object_literal_from_graph(graph, title_uri)
        abstract = extract_object_literal_from_graph(graph, abstract_uri)
        return title, abstract

def _decide_to_override(ask_to_override: bool):
        if not ask_to_override: return False
        x = input("Should we accept the best result and override the input file's discipline [y/n]?")
        return x != None and x.strip().lower() == 'y'

def _override_and_move_input_file(input_file_path, best_concept: str, move_original: bool, file_format: str='.ttl'):
    graph = input_file_to_graph(input_file_path)
    graph.bind("ocs", OCS_URI)

    paper_URI = list(graph.triples((None, URIRef("http://www.w3.org/1999/02/22-rdf-syntax-ns#type"), URIRef(PAPER_TYPE_URI))))[0][0]
    has_discipline_URI = URIRef(HAS_DISCIPLINE_URI)
    best_concept_URI = URIRef(OCS_URI + best_concept)

    _update_has_discipline_result(graph, paper_URI, has_discipline_URI, best_concept_URI)

    _move_updated_file(graph, input_file_path, move_original, file_format)


def _update_has_discipline_result(graph: Graph, paper_URI: URIRef, has_discipline_URI: URIRef, best_concept_URI: URIRef):
...
def _move_updated_file(graph: Graph, input_file_path: str, move_original: bool, file_format: str='ttl'):
...
def _get_best_concept_and_score(query_result: str, labels_to_concepts_names: Dict[str, str], label_colname: str):
...
def _replace_title_abstract_placeholders(val, title_val, abstract_val, title_placeholder, abstract_placeholder):

    if type(val) == dict:
        dictionary = val
        for k, v in dictionary.items():
            if type(v) != str:
                _replace_title_abstract_placeholders(v, title_val, abstract_val, title_placeholder, abstract_placeholder)
            else:
